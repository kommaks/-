...
void ArraysParameters(const double a, const double b, const double c, const int N, const double T_l, const double T_r,
    const double Nd, vector <double>& ACp, vector <double>& ADfCp, vector <double>& ADensity, vector <double>& ADfDensity,
    vector <double>& ALambda, vector <double>& ADfLambda)
{
    ACp.resize(N + 1);
    ADfCp.resize(N + 1);
    ADensity.resize(N + 1);
    ...
    */
    for (int i = 0; i < Nd + 1; i++) {
        ...
        ACp[i] = Cp(T_l, a);
        ADfCp[i] = DfCp(T_l, a);
        ADensity[i] = Density(T_l, a);
        ADfDensity[i] = DfDensity(T_l, a);
        ALambda[i] = Lambda(T_l, a, b, c);
        ADfLambda[i] = DfLambda(T_l, a, b);
    }
    for (int i = Nd + 1; i < N + 1; i++) {
        //cout << r[i] << endl;
        ACp[i] = Cp(T_r, a);
        ADfCp[i] = DfCp(T_r, a);
        ADensity[i] = Density(T_r, a);
        ADfDensity[i] = DfDensity(T_r, a);
        ALambda[i] = Lambda(T_r, a, b, c);
        ADfLambda[i] = DfLambda(T_r, a, b);
    }
    //Plot graphics of Parameters as a function of Temperature
    ofstream Parameters;
    Parameters.open("C:/Users/user/source/Научная работа/DropletWarming/Data/Parameters.dat");
    Parameters << "TITLE=\"" << "Graphics" << "\"" << endl;
    Parameters << R"(VARIABLES= "i", "Cp", "DfCp", "rho", "Dfrho", "Lambda", "DfLambda")" << endl;
    for (int i = 0; i < N + 1; i++) {
        //cout << r[i] << endl;
        Parameters << i << " " << ACp[i] << " " << ADfCp[i] << " " << ADensity[i] << " " << ADfDensity[i] << " "
            << ALambda[i] << " " << ADfLambda[i] << "\n";
    }
    Parameters.close();
}
...
{
    return (2. / (r[j + 1] - r[j - 1]) / pow(r[j], 2.)) * pow((r[j] + r[j - 1]) / 2., 2.) * (-0.5 * (ADfLambda[j] + ADfLambda[j - 1])
...
double DfCenter(vector<double>& r, vector<double>& T_next, vector <double>& ACp, vector <double>& ADfCp, vector <double>& ADensity,
    ...
    const double a, const double b, const double c, const double d, int j, const double dt, const double dtau)
{
    //if (a == pow(10, -9))
    //{
    ...
    //else
    //{
    return (2. / (r[j + 1] - r[j - 1]) / pow(r[j], 2.)) * (pow((r[j + 1] + r[j]) / 2., 2.) * (0.5 * (ADfLambda[j + 1] + ADfLambda[j])
        * (T_next[j + 1] - T_next[j]) / (r[j + 1] - r[j]) - 0.5 * (ALambda[j + 1] + ALambda[j]) / (r[j + 1] - r[j]))
        - pow((r[j] + r[j - 1]) / 2., 2.) * (0.5 * (ADfLambda[j] + ADfLambda[j - 1]) * (T_next[j] - T_next[j - 1])
            / (r[j] - r[j - 1]) + 0.5 * (ALambda[j] + ALambda[j - 1]) / (r[j] - r[j - 1])))
        - (ACp[j] * ADensity[j] + (ADfCp[j] * ADensity[j] + ACp[j] * ADfDensity[j]) * T_next[j]) / dt;
    //}
}
//Setting j+1 element of matrix
...
{
    return (2. / (r[2] - r[0]) / pow(r[1], 2.)) * (pow((r[2] + r[1]) / 2., 2.) * (0.5 * (ADfLambda[2] + ADfLambda[1]) * (T_next[2] - T_next[1])
        / (r[2] - r[1]) - 0.5 * (ALambda[2] + ALambda[1]) / (r[2] - r[1]))) - (ACp[1] * ADensity[1] + (ADfCp[1] * ADensity[1]
            + ACp[1] * ADfDensity[1]) * T_next[1]) / dt;
}
...
void Jacobian(vector<vector<double>>& J, vector<double>& r, vector<double>& T_next, vector <double>& ACp, vector <double>& ADfCp,
    ...
    const double a, const double b, const double c, const double d, const double dt, const double dtau, const int const_params)
{
    
    if (const_params == 0)
    {
            J[1][1] = DfCenter(r, T_next, ACp, ADfCp, AD, ADfD, AL, ADfL, a, b, c, d, 1, dt, dtau);
            J[1][2] = DfRight(r, T_next, AL, ADfL, a, b, c, 1);
            ...
            J[N_minus][N_minus - 1] = DfLeft(r, T_next, AL, ADfL, a, b, c, N_minus);
    ...
    else if (const_params == 2 || const_params == 1)
    {
        ...
        J[1][1] = AdiabaticBoundaryDfs(r, T_next, ACp, ADfCp, AD, ADfD, AL, ADfL, a, b, c, dt, dtau);
        J[1][2] = DfRight(r, T_next, AL, ADfL, a, b, c, 1);
        for (int j = 2; j < N_minus; j++) {
            J[j][j - 1] = DfLeft(r, T_next, AL, ADfL, a, b, c, j);
            J[j][j] = DfCenter(r, T_next, ACp, ADfCp, AD, ADfD, AL, ADfL, a, b, c, d, j, dt, dtau);
...
void F(vector<double>& f, vector<double>& T_cur, vector<double>& T_next, vector <double>& ACp, vector <double>& ADensity, vector <double>& ALambda,
    const int N, vector<double>& r, const double a, const double b, const double c, const double d, const double dt, const int const_params)
{
    if (const_params == 0)
    {
        for (int j = 1; j < N; j++)
        {
            ...
            f[j] = -((2. / (r[j + 1] - r[j - 1]) / pow(r[j], 2.)) * (pow((r[j + 1] + r[j]) / 2., 2.) * 0.5 * (ALambda[j + 1] + ALambda[j])
                ...
                * 0.5 * (ALambda[j] + ALambda[j - 1]) * (T_next[j] - T_next[j - 1]) / (r[j] - r[j - 1]))
                - (ACp[j] * ADensity[j]) * (T_next[j] - T_cur[j]) / dt);
        }
    }
    else if (const_params == 2 || const_params == 1)
    {
        f[1] = -((2. / (r[2] - r[0]) / pow(r[1], 2.)) * pow((r[2] + r[1]) / 2., 2.)
            * (0.5 * (ALambda[2] + ALambda[1]) * (T_next[2] - T_next[1]) / (r[2] - r[1])) - (ACp[1] * ADensity[1]) * (T_next[1] - T_cur[1]) / dt);
        //cout << "f[0]" << f[0] << "\n";
        for (int j = 2; j < N; j++)
        {
            //cout << "(T_next[j] - T[j][n])" << (T_next[j] - T[j][n]) << endl;
            f[j] = -((2. / (r[j + 1] - r[j - 1]) / pow(r[j], 2.)) * (pow((r[j + 1] + r[j]) / 2., 2.) * 0.5 * (ALambda[j + 1] + ALambda[j])
                * (T_next[j + 1] - T_next[j]) / (r[j + 1] - r[j]) - pow((r[j] + r[j - 1]) / 2., 2.) * 0.5 * (ALambda[j] + ALambda[j - 1])
    ...
    const double T_l,const double T_r, const double dtau, const int Nd)
{
    ...
    ArraysParameters(a, b, c, N, T_l, T_r, Nd, ACp, ADfCp, ADensity, ADfDensity, ALambda, ADfLambda);
    ...
    { 
        ...
        F(nevyaz, T_cur, T_next, ACp, ADensity, ALambda, N, r, a, b, c, d, dt, const_params);
        //Collecting Nevyazka
        if (n % 10 == 0 || n < 20)
        {
        ...
        while (mod_nevyaz > pow(10, -2))
        {
            mod_nevyaz = 0;
            Jacobian(J, r, T_next, ACp, ADfCp, ADensity, ADfDensity, ALambda, ADfLambda, N_minus, a, b, c, d, dt, const_params);
            cout << "s " << s << endl;
            ...
            if (n % 100 == 0 || n < 20)
            {
                OutCurrentTemp.open("C:/Users/user/source/Научная работа/DropletWarming/Data/Temp_" + to_string(n + 1) + ".dat");
